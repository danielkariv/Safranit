...
import { fetchOwnedBooks, verifySession } from "@/libs/data";
...
export default async function Home({
  ...
  const { books, pages } = await fetchOwnedBooks(searchQuery, currentPage, itemsPerPage, sessionData); // Fetch books from API or backend
  ...
  const renderPagination = () => {
    ...
    if (currentPage > 3) {
      pagination.push(
        <a
          key="first"
          href={`/owned-books?page=1&query=${encodeURIComponent(searchQuery)}`}
    ...
    if (currentPage > 1) {
      pagination.push(
        <a
          key="prev"
          href={`/owned-books?page=${currentPage - 1}&query=${encodeURIComponent(searchQuery)}`}
          className="join-item btn"
        >
          &lt;
        </a>
    ...
    if (currentPage > 2) {
      pagination.push(
        <a
          key={currentPage - 2}
          href={`/owned-books?page=${currentPage - 2}&query=${encodeURIComponent(searchQuery)}`}
          className="join-item btn"
        >
          {currentPage - 2}
        </a>
    ...
    if (currentPage < pages) {
      pagination.push(
        <a
          key="last"
          href={`/owned-books?page=${pages}&query=${encodeURIComponent(searchQuery)}`}
          className="join-item btn"
        >
          {pages}
        </a>
  ...
  return (
    <div className="max-w-screen-lg w-full mx-auto flex flex-col items-center">
      ...
      <div className="flex w-full items-center justify-between p-4">
        <h2 className="font-bold text-xl">Owned Books:</h2>
        <form
          method="get"
          action="/owned-books"
